...
namespace SistemaEscolar.Services.Usuarios
{
 ...
 var roles = await _ctx.Roles.Include(r=>r.RolPermisos).ThenInclude(rp=>rp.Permiso).ToListAsync();
 return roles.Select(r=> new RolDTO{ Id=r.Id, Nombre=r.Nombre, Permisos = r.RolPermisos.Select(p=>p.Permiso.Codigo).ToList() }).ToList(); }
 public async Task<RolDTO?> GetByIdAsync(int id){
 var r = await _ctx.Roles.Include(x=>x.RolPermisos).ThenInclude(p=>p.Permiso).FirstOrDefaultAsync(x=>x.Id==id); if(r==null) return null; return new RolDTO{ Id=r.Id, Nombre=r.Nombre, Permisos = r.RolPermisos.Select(p=>p.Permiso.Codigo).ToList() }; }
 ...
 public async Task<bool> DeleteAsync(int id, int usuarioId, string ip){ var rol = await _ctx.Roles.FindAsync(id); if(rol==null) return false; if(await _ctx.UsuarioRoles.AnyAsync(ur=>ur.RolId==id)) return false; _ctx.Roles.Remove(rol); await _ctx.SaveChangesAsync(); await _bitacora.RegistrarAsync(usuarioId,$"Eliminar rol {rol.Nombre}","Seguridad","{ip}"); return true; }
 }
}
